"""
Bronze layer: raw user data ingestion from Parquet volume.
...
@dp.table(
    comment=(
        "Raw user records ingested from SaaS landing zone Parquet files. "
        "Append-only. Schema evolution enabled via Auto Loader. "
        "Source: /Volumes/aibuilder_saas_demo/raw_data/saas_data/users/"
    ),
    cluster_by=["user_id"],
